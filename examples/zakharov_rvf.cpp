...
struct deref_if_needed_ftor;

// Deref implementation for std::vector (identity)
template<typename T>
constexpr std::vector<T>& tag_invoke(deref_if_needed_ftor, std::vector<T>& v) noexcept {
    return v;
}

template<typename T>
constexpr const std::vector<T>& tag_invoke(deref_if_needed_ftor, const std::vector<T>& v) noexcept {
    return v;
}
